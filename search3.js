		...
		pickerBtn: $(".picker"),
				...
				this.els.hotList.html(function () {
					...
					$.each(_this.hot.list, function (index, item) {
						str += '<a href="/search?s=' + item + '&u=1&p=video">'
				...
				this.els.button.click(function () {
					...
					if (_this.els.input.val() != "") {
						if (searchArr[_this.searchIndex] == "视频") {
							window.location.href = "/search?s=" + _this.els.input.val() +"&u=1&p=video";
						}
						if (searchArr[_this.searchIndex] == "图片") {
							window.location.href = "/search?s=" + _this.els.input.val() + "&u=1&p=image";
						}
